...
CREATE TABLE IF NOT EXISTS etl.countries_gdp_update_log (
	...
	country						VARCHAR(50),
	prev_gdp_usd_billions		FLOAT,
	curr_gdp_usd_billions		FLOAT,
	period						DATE,
...
BEGIN
	...
	new_hash := MD5(country_name || COALESCE(gdp_region, '') || round(gdp_value::numeric, 2)::text || gdp_period::text);
	
	-- Retrieve existing record, if any, based on the country and period
	SELECT * INTO existing_record
	FROM etl.countries_gdp
	WHERE country = country_name 
		AND period = gdp_period;
	
	RAISE NOTICE 'New hash: %, Existing hash: %', new_hash, existing_record.data_hash;
	...
	IF existing_record.country_gdp_id IS NULL THEN
		...
		VALUES(country_name, gdp_region, gdp_value, gdp_period, CURRENT_TIMESTAMP, new_hash, p_batch_id)
		RETURNING country_gdp_id INTO existing_record.country_gdp_id;
		
		-- Insertion Log
		INSERT INTO 
			etl.countries_gdp_update_log(
				action_taken, 
				country, 
				curr_gdp_usd_billions, 
				period, 
				country_gdp_id, 
				batch_id)
		VALUES (
			...
			gdp_value, 
			gdp_period, 
			existing_record.country_gdp_id, 
			p_batch_id);
	...
	ELSEIF new_hash != existing_record.data_hash THEN
		...
		VALUES (
			'UPDATE', 
			country_name, 
			existing_record.gdp_usd_billions, 
			gdp_value, 
			gdp_period, 
			existing_record.country_gdp_id, 
			p_batch_id);
